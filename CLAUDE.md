...
- **Verified end-to-end on this machine (Docker Desktop 4.89, Windows)**: the Vault chain (vault → vault-seed → vault-agent → all secret-consuming containers, including the `$$REDIS_AUTH` escaping) works exactly as designed — confirmed by actually running `docker compose up vault-agent` then `docker compose up -d` (twice, including a run from completely clean images/volumes after a full Docker Desktop restart) and inspecting the rendered `secrets.env`, container logs, and a `redis-cli` auth check.
- **Traefik's routing does not work on this machine, root-caused (not just worked around), status: unresolved.** `docker-compose.yml` itself is correct — this is a client-side bug/incompatibility in Traefik's embedded Docker SDK against this specific Docker Desktop install, not a config mistake:
  ...
  2. Bind-mounting that real path directly into Traefik gets further — `docker.proxy.sock`'s own `/_ping` and `/version` respond correctly (verified with `curl --unix-socket`, `Api-Version: 1.55`) — but Traefik's Docker provider still logs `Failed to retrieve information of the docker client and server host: client version 1.24 is too old`. It's sending the request as API v1.24 and never negotiating up, even though the daemon serves the correct version on `/_ping`.
  ...
  - Do not hardcode `/run/guest-services/docker.proxy.sock` into the committed `docker-compose.yml` — that path is Windows/Docker-Desktop-4.89-internal, not portable (breaks Linux/Mac/CI, where plain `/var/run/docker.sock` is correct and this whole problem doesn't exist). A local, gitignored `docker-compose.override.yml` is the right place for any machine-specific workaround, if one is ever found. Untried alternative if this needs to actually work locally: swap Traefik for a different Docker-label-based reverse proxy (e.g. Caddy) that might not hit the same client negotiation bug.

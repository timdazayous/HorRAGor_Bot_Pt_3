...
# Full stack with monitoring + Vault + Caddy (fresh start / after `docker compose down -v`)
...
### Local Docker stack — Vault + Caddy (learning exercise, not production)

`docker-compose.yml` also runs HashiCorp Vault (dev mode) and Caddy (`caddy-docker-proxy`), added purely to learn the tools (course-driven, not a project requirement) — see the "Vault & Caddy" section of README.md for the full picture. Key points if you touch this:
...
- All of `caddy`/`vault`/`vault-seed`/`vault-agent` and every proxied service's `caddy`/`caddy.reverse_proxy` labels are dev-only shortcuts (`http://` prefix disables Caddy's automatic HTTPS, admin API on `:2019` unauthenticated, Vault `-dev` in-memory storage with a fixed root token) — flagged as such in the README, don't carry these patterns into anything resembling production without hardening them first.
- **Verified end-to-end on this machine (Docker Desktop 4.89, Windows)**: the Vault chain (vault → vault-seed → vault-agent → all secret-consuming containers, including the `$$REDIS_AUTH` escaping) works exactly as designed. Also verified end-to-end: real HTTP routing through `caddy` to all 6 services (`api`, `langfuse`, `grafana`, `prometheus`, `uptime`, `vault` sub-hostnames) — actual 200/302/307 app-level responses, not just "container started".
- **Why Caddy and not Traefik (the original choice)**: Traefik was tried first and its Docker provider failed reproducibly on this machine, root-caused (not just worked around) before switching:
  1. `/var/run/docker.sock` doesn't exist inside Docker Desktop's internal `docker-desktop` WSL2 distro on this version — confirmed via `wsl -d docker-desktop`. The real engine socket lives at `/run/guest-services/docker.proxy.sock`.
  2. Bind-mounting that real path directly into Traefik gets further — `docker.proxy.sock`'s own `/_ping` and `/version` respond correctly (verified with `curl --unix-socket`, `Api-Version: 1.55`) — but Traefik's Docker provider still logs `Failed to retrieve information of the docker client and server host: client version 1.24 is too old`. It sends the request as API v1.24 and never negotiates up, even though the daemon serves the correct version on `/_ping`.
  3. Ruled out as the cause: the exact socket path (tried both direct mount and a `tecnativa/docker-socket-proxy` relay — same v1.24 request logged on the proxy's own access log), the `DOCKER_API_VERSION` env var (present inside the container, provider ignores it), and the Traefik version (identical failure on v3.2.5 and v3.5).
  4. `lucaslorentz/caddy-docker-proxy` was tested against the exact same socket path before committing to the switch — connected immediately (no version-negotiation issue), correctly discovered a labeled test container, and routed real HTTP traffic to it. That's when Traefik got replaced in `docker-compose.yml`.
  - **Don't hardcode `/run/guest-services/docker.proxy.sock` into the committed `docker-compose.yml`** — that path is Windows/Docker-Desktop-4.89-internal, not portable (breaks Linux/Mac/CI, where plain `/var/run/docker.sock` is correct and none of this problem exists). The fix lives in a local, gitignored `docker-compose.override.yml` instead — on this machine specifically: `services: { caddy: { volumes: !override ["/run/guest-services/docker.proxy.sock:/var/run/docker.sock:ro", "caddy_data:/data", "caddy_config:/config"] } }`. Note `!reset` (the Compose Specification tag that sounds like it should do this) did *not* work as documented here — it silently produced an empty `volumes:` list on this Compose version (v5.5.0); `!override` did work. If Caddy logs `Docker ping failed` / `no such file or directory` on a fresh machine, find the real socket path first (`wsl -d docker-desktop -e sh -c "find / -xdev -name 'docker*.sock'"` on Windows) before assuming the override's hardcoded path still applies.
...
**Logs → Loki (learning exercise, same spirit as Vault/Caddy)**: `setup_logging()` writes a *third* Loguru sink besides console and the human-readable `horragor.log` — `horragor.jsonl`, one **flat** JSON object per line (`time`, `level`, `message`, `module`, `function`, `line`). Deliberately not Loguru's `serialize=True` (which nests everything under `record.level.name`/`record.message` — awkward for LogQL's `| json`, which flattens nested keys with `_` rather than giving you the paths you'd expect). The custom `_json_line()` formatter builds the JSON itself and stashes it in `record["extra"]["json_line"]`, returning `"{extra[json_line]}\n"` as the format string — Loguru's `format` callable must return a `.format()` template, not the final string, so embedding raw `{`/`}` from real JSON directly would break rendering.

`promtail` (docker-compose.yml) scrapes `logs/horragor.jsonl` as a **static file target** (`__path__`), not via `docker_sd_configs`/`/var/run/docker.sock` like the course material shows — same reasoning as the Traefik→Caddy socket saga above: avoid depending on that socket at all where a simpler alternative exists. Labels kept to `job`/`app`/`env` only (low cardinality — see the "piège de la cardinalité" from the course PDF); never promote per-request values (user_id, request_id) to labels. Verified end-to-end on this machine: real API logs flow into Loki, `{job="horragor-api"} | json | level="WARNING"` and the dashboard's `count_over_time(... | json [1m])` both returned correct results, and the provisioned alert rule (`monitoring/grafana/provisioning/alerting/rules.yml`) loaded into Grafana without error and was actively evaluated every minute (checked via Grafana's own `/api/v1/provisioning/alert-rules` and the grafana container logs, not just "the YAML looks right").
